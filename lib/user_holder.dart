...
class UserHolder {
  ...
  void registerUser(String username, String phone, String email) {
    User user =
        User(name: username.capitalizeName(username), phone: phone, email: email);
  ...
  User registerUserByEmail(String fullName, String email) {
    User user =
        User(name: fullName.capitalizeName(fullName), email: email, phone: null);
    if (_users.containsKey(user.login)) {
  ...
  User findUserInFriends(String fullName, User user) {
    User toFindFriendOf = _users[fullName.capitalizeName(fullName)];
    if (toFindFriendOf == null) {
      throw Exception('No user with name ${fullName.capitalizeName(fullName)}!');
    }
...
//      List<List<dynamic>> singleUserDate = const CsvToListConverter() .convert(userData);
//    });
//    """
//      Eric Freeman;
